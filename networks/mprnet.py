...
flow_warping = LiteFlowNet.backwarp
...
class Model(nn.Module):
    def __init__(self, opts, scales=[1]):
        ...
        self.frame_restorer = MPRNet(in_c=21, out_c=3)
        if self.ifAlign:
            if self.ifAggregate:
                self.flow_estimator = Restormer(inp_channels=21, out_channels=13)
                self.frame_restorer = MPRNet(in_c=3, out_c=3)
            else:
                self.flow_estimator = Restormer(inp_channels=21, out_channels=12)

    def forward(self, inputs, ifInferece=True):
        bn, ch_f, sq, h, w = inputs.shape
        flow_list = []
        frame_center = inputs[:,:,sq//2]
        if self.ifAlign:
            seq_around = inputs[:,:,[i for i in range(sq) if i != sq//2]]

            input_ = inputs.view(bn, -1, h, w)

            scales = self.scales if not ifInferece else [1]

            for j, scale in enumerate(scales):
                input_scaled = F.interpolate(input_, scale_factor=self.scales[j], mode="bilinear", align_corners=False)
                flow_inward = self.flow_estimator(input_scaled)
                flow_list.append(flow_inward)
            flow_inward = flow_list[-1]
            if self.ifAggregate:
                flow_inward, alpha = flow_inward[:,:-1], flow_inward[:,-1:].unsqueeze(2)

            flow_inward = flow_inward.contiguous().view(bn, 2, sq-1, h, w)
            warp_inward = [flow_warping(seq_around[:,:,i], flow_inward[:,:,i]) for i in range(sq-1)]


            seq_input = torch.cat([frame.unsqueeze(2) for frame in warp_inward[:3]]
                                   +[frame_center.unsqueeze(2)]+
                                  [frame.unsqueeze(2) for frame in warp_inward[3:]],2)

            if self.ifAggregate:
                seq_input = softMedian(seq_input, dim=2, alpha=alpha)
            else:
                seq_input = seq_input.view(bn, -1, h, w)

        else:
            seq_input = inputs.view(bn, -1, h, w)


        out = self.frame_restorer(seq_input)


        return out + frame_center, flow_list
