...
class Model(nn.Module):
    def __init__(self, dim=32, inch=3, outch=3, ifSoftMedian=True):
        super().__init__()
        self.ifSoftMedian = ifSoftMedian
        self.input = nn.Conv3d(inch, dim, 3, 1, 1)
        self.res1 = ResBlock(dim, 20)
        
        if not ifSoftMedian:
            self.out = nn.Conv2d(dim*7, outch, 3, 1, 1)
        else:
            self.out = nn.Conv3d(dim, inch, 3, 1, 1)
