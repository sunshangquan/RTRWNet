...
def backwarp(tenInput, tenFlow, ):
    ...
    if str(tenFlow.shape) not in backwarp_tenGrid:
        ...
        backwarp_tenGrid[str(tenFlow.shape)] = torch.cat([ tenHor, tenVer ], 1).to(tenFlow)
        print("create backwarp_tenGrid")
    # end
...
class LightFlowNet(torch.nn.Module):
    ...
    def forward(self, tenFirst, tenSecond):
        _, _, intHeight, intWidth = tenFirst.shape
        
        intPreWidth = int(math.floor(math.ceil(intWidth / 32.0) * 32.0))
        intPreHeight = int(math.floor(math.ceil(intHeight / 32.0) * 32.0))
        ...
        tenFlow[:, 0, :, :] *= float(intWidth) / float(intPreWidth)
        tenFlow[:, 1, :, :] *= float(intHeight) / float(intPreHeight)
        return tenFlow
    # end
# end
