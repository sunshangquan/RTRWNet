...
def get_residue(frame_i0):
    frame_i0_min = torch.min(frame_i0, dim=1)
    frame_i0_min = frame_i0_min[0]
    frame_i0_max = torch.max(frame_i0, dim=1)
    frame_i0_max = frame_i0_max[0]
    frame_i0_residue = frame_i0_max - frame_i0_min
    [b, h, w] = frame_i0_residue.shape
    frame_i0_residue = frame_i0_residue.view(b, 1, h, w)

    frame_i0_residue = torch.cat((frame_i0_residue, frame_i0_residue, frame_i0_residue), 1)
    return frame_i0_residue
...
def loss_setting(opt, opts):
    criterion_list = []

    if opt.type_rain_variable== 'L2':
        criterion = nn.MSELoss(size_average=True)
    elif opt.type_rain_variable == 'L1':
        criterion = nn.L1Loss(size_average=True)
    elif opt.type_rain_variable == 'SSIM':
        criterion = SSIM()
    else:
        raise Exception("Unsupported criterion %s" %opts.loss)
    criterion_list.append(criterion)

    if opt.type_rain_rect== 'L2':
        criterion = nn.MSELoss(size_average=True)
    elif opt.type_rain_rect == 'L1':
        criterion = nn.L1Loss(size_average=True)
    elif opt.type_rain_rect == 'SSIM':
        criterion = SSIM()
    else:
        raise Exception("Unsupported criterion %s" %opts.loss)
    criterion_list.append(criterion)
    ...
    if opt.type_haze_variable== 'L2':
        criterion = nn.MSELoss(size_average=True)
    elif opt.type_haze_variable == 'L1':
        criterion = nn.L1Loss(size_average=True)
    elif opt.type_haze_variable == 'SSIM':
        criterion = SSIM()
    else:
        raise Exception("Unsupported criterion %s" %opts.loss)
    criterion_list.append(criterion)
    ...
    if opt.type_haze_rect== 'L2':
        criterion = nn.MSELoss(size_average=True)
    elif opt.type_haze_rect == 'L1':
        criterion = nn.L1Loss(size_average=True)
    elif opt.type_haze_rect == 'SSIM':
        criterion = SSIM()
    else:
        raise Exception("Unsupported criterion %s" %opts.loss)
    criterion_list.append(criterion)

    if opt.type_flow_variable== 'L2':
        criterion = nn.MSELoss(size_average=True)
    elif opt.type_flow_variable == 'L1':
        criterion = nn.L1Loss(size_average=True)
    elif opt.type_flow_variable == 'SSIM':
        criterion = SSIM()
    else:
        raise Exception("Unsupported criterion %s" %opts.loss)
    criterion_list.append(criterion)

    if opt.type_final_variable== 'L2':
        criterion = nn.MSELoss(size_average=True)
    elif opt.type_final_variable == 'L1':
        criterion = nn.L1Loss(size_average=True)
    elif opt.type_final_variable == 'SSIM':
        criterion = SSIM()
    else:
        raise Exception("Unsupported criterion %s" %opts.loss)
    criterion_list.append(criterion)
    ...
    if opt.type_final_rect== 'L2':
        criterion = nn.MSELoss(size_average=True)
    elif opt.type_final_rect == 'L1':
        criterion = nn.L1Loss(size_average=True)
    elif opt.type_final_rect == 'SSIM':
        criterion = SSIM()
    else:
        raise Exception("Unsupported criterion %s" %opts.loss)
    criterion_list.append(criterion)

    if opt.type_final_w_h_rect== 'L2':
        criterion = nn.MSELoss(size_average=True)
    elif opt.type_final_w_h_rect == 'L1':
        criterion = nn.L1Loss(size_average=True)
    elif opt.type_final_w_h_rect == 'SSIM':
        criterion = SSIM()
    else:
        raise Exception("Unsupported criterion %s" %opts.loss)
    criterion_list.append(criterion)
...
def mykaiming_normal_(tensor, a=0, mode='fan_in', nonlinearity='leaky_relu'):
    fan  = torch.nn.init._calculate_correct_fan(tensor, mode)
    gain = torch.nn.init. calculate_gain(nonlinearity, a)
    std  = gain / math.sqrt(fan)
    with torch.no_grad():
        return tensor.normal_(0, std*0.1)

def weight_init(m, factor=1):
    classname = m.__class__.__name__
    if classname.find('Conv2d') != -1:
        mykaiming_normal_(m.weight)
        print(type(m.bias))

        if str(type(m.bias)) != 'NoneType':
            m.bias.data.fill_(0)
...
def weight_init(m, factor=1):
    ...
    if classname.find('Conv2d') != -1:
        ...
        if str(type(m.bias)) != '''<class 'NoneType'>''':
            m.bias.data.fill_(0)
...
def tensor2img(img_t):
    ...
    if len(shape) == 4:
        ...
        seq = shape[2]
        for sq in range(seq):
            img = img_t[0,:,sq].detach().to("cpu").numpy()
            img = np.transpose(img, (1, 2, 0))
            imgs.append(img)
        return imgs[seq//2]

def save_model(three_dim_model, flow_model, optimizer, optimizer_flow, lr_scheduler, lr_scheduler_flow, opts):
    ...
    state_dict = {
            'three_dim_model': three_dim_model.state_dict(),
            'flow_model': flow_model.state_dict(),
            'optimizer' : optimizer.state_dict(),
            'optimizer_flow': optimizer_flow.state_dict(),
            'lr_scheduler': lr_scheduler.state_dict(),
            'lr_scheduler_flow': lr_scheduler_flow.state_dict(),
        }

    model_filename = os.path.join(opts.model_dir, "model_epoch_%d.pth" % three_dim_model.epoch)
    print("Save %s" % model_filename)
    torch.save(state_dict, model_filename)


def load_model(three_dim_model, flow_model, optimizer, optimizer_flow, lr_scheduler, lr_scheduler_flow, opts, epoch):
...
#    state_dict_w_module = {}
#    for key in state_dict['three_dim_model']:
#        key_w_module = key.split('.')
#        state_dict_w_module['.'.join([key_w_module[0], 'module'] + key_w_module[1:])] = state_dict['three_dim_model'][key]
#    three_dim_model.load_state_dict(state_dict_w_module)
    three_dim_model.load_state_dict(state_dict['three_dim_model'])
    flow_model.load_state_dict(state_dict['flow_model'])
    if epoch > 0:
        optimizer.load_state_dict(state_dict['optimizer'])
        optimizer_flow.load_state_dict(state_dict['optimizer_flow'])
#        lr_scheduler.load_state_dict(state_dict['lr_scheduler'])
#        lr_scheduler_flow.load_state_dict(state_dict['lr_scheduler_flow'])
    ...
    three_dim_model.epoch = epoch ## reset model epoch

    return three_dim_model, flow_model, optimizer, optimizer_flow, lr_scheduler, lr_scheduler_flow
...
def create_data_loader(data_set, opts, mode):
    ...
    batch_size = 1 if 'test' in mode else opts.batch_size
    if 'train' in mode:
        total_samples = opts.train_epoch_size * batch_size
        ...
        data_loader = DataLoader(dataset=data_set, num_workers=opts.threads, batch_size=batch_size, sampler=sampler, pin_memory=False)
    else:
        total_samples = opts.valid_epoch_size * batch_size
        data_loader = DataLoader(dataset=data_set, num_workers=opts.threads, batch_size=batch_size, shuffle=False, pin_memory=False)
...
def align_to_f(frame_i0, divide):
    [b, c, h, w] = frame_i0.shape

    h_pad = math.ceil(h / divide) * divide
    w_pad = math.ceil(w / divide) * divide
    frame_i0_pad = F.pad(frame_i0, pad = [0, w_pad-w, 0, h_pad-h], mode='replicate')
    # print("Padding", w_pad-w, h_pad-h)

    return frame_i0_pad, h_pad-h, w_pad-w

def compute_psnr(im1, im2, MAX=255.):
    # im1, im2 = im1.clip(0,1)*255, im2.clip(0,1)*255
    mse = ((im1 - im2)**2).mean()
    if mse == 0:
        return float('inf')
